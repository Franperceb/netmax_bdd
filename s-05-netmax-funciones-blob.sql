...
--Funcion que obtiene BLOB del fragmento 1
create or replace function get_remote_archivo_f1_by_id(v_id in number , v_num_archivo in number)
return blob is
pragma autonomous_transaction;
v_temp_archivo blob;
begin
...
from archivo_programa_f1 where programa_id = v_id and num_archivo = v_num_archivo;
--obtiene el registro de la tabla temporal y lo regresa como blob
select archivo into v_temp_archivo from ts_archivo_programa_1 where programa_id = v_id and num_archivo = v_num_archivo;
--elimina los registros de la tabla temporal una vez que han sido obtenidos.
delete from ts_archivo_programa_1;
commit;
...
--Funcion que obtiene BLOB del fragmento 2
create or replace function get_remote_archivo_f2_by_id(v_id in number , v_num_archivo in number)
return blob is
pragma autonomous_transaction;
v_temp_archivo blob;
begin
...
from archivo_programa_f2 where programa_id = v_id and num_archivo = v_num_archivo;
--obtiene el registro de la tabla temporal y lo regresa como blob
select archivo into v_temp_archivo from ts_archivo_programa_2 where programa_id = v_id and num_archivo = v_num_archivo;
--elimina los registros de la tabla temporal una vez que han sido obtenidos.
delete from ts_archivo_programa_2;
commit;
return v_temp_archivo;
