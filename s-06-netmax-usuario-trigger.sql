...
begin
  ...
  when inserting then
    ...
    insert into usuario_f3(usuario_id, num_tarjeta, password)
    values(:new.usuario_id, :new.num_tarjeta, :new.password);
    
    if :new.vigente not in(0,1) then
      raise_application_error(-20010,
        'Se ha detectado un problema con la inserción o eliminación del registro, ya que no se ajusta al'
        ||'esquema fragmentación horizontal primaria.'
        ||'VIGENTE = ' ||:new.vigente
    ...
    if :new.vigente = 0 then
      ...
      raise_application_error(-20010,
        'Se ha detectado un problema con la inserción o eliminación del registro, ya que no se ajusta al esquema de '
